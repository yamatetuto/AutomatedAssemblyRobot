        ...
        async with self._modbus_lock:
            try:
                ...
                position = await asyncio.to_thread(
                    self.controller.instrument.read_long,
                    self.controller.REG_CURRENT_POS,
                    functioncode=3,
                    signed=True
                )
                alarm = await asyncio.to_thread(
                    self.controller.instrument.read_register,
                    self.controller.REG_CURRENT_ALARM,
                    functioncode=3
                )
                device_status = await asyncio.to_thread(
                    self.controller.instrument.read_register,
                    self.controller.REG_DEVICE_STATUS,
                    functioncode=3
                )
                
                servo_on = bool((device_status >> self.controller.BIT_SERVO_READY) & 1)
