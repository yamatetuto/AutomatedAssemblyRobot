...
|------|--------|------|
| **動作範囲** | 0 ~ 4 mm | 開閉ストローク |
| **位置決め精度** | ±0.01 mm | 繰り返し精度 |
| **最大速度** | 200 mm/s | 推奨: 50-100 mm/s |
| **最大把持力** | 50 N | 過負荷注意 |
...
| 0x1000~ | ポジションテーブル | 各ポジションの座標値 |

#### ステータスレジスタ (読み取り)
| アドレス | 名称 | 説明 |
|----------|------|------|
| 0x9000 | 現在位置 (PNOW) | 現在の位置 (0.01mm単位) |
...
5. **ストローク制限**: 0~4mm範囲外への移動禁止

#### 推奨動作シーケンス
```
1. サーボON (0x1000)
2. 原点復帰 (0x1010) → HEND=1を待機
